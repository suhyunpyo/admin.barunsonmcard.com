...
namespace Barunn.MobileInvitation.Web.Admin.Controllers
{
    ...
    {
        public NoticeController(ILogger<NoticeController> logger, IOptions<BarunnConfig> barunnConfig, BarunsonContext barunsonDb, BarShopContext barshopDb)
             : base(logger, barunnConfig, barunsonDb, barshopDb)
        { }

        public async Task<IActionResult> Index(NoticeSearchViewModel model)
        {
            model.RouteAction = "Index";
            model.RouteController = "Notice";
            ...
            var query = from a in _barunsonDb.TB_Boards
                        ...
                        select new NoticeSearchViewModel.NoticeItem
                        {
                            Board_ID = a.Board_ID,
                            Title = a.Title,
                            Hits = a.Hits,
        ...
        public IActionResult Add(string returnUrl)
        {
            ...
            model.ReturnUrl = returnUrl ?? Url.Action("Index", "Notice");
        ...
        {
            var query = from a in _barunsonDb.TB_Boards
                        where a.Board_ID == id && a.Board_Category == "N"
                        select a;
            ...
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    item = await (from a in _barunsonDb.TB_Boards
                                  where a.Board_ID == model.Board_ID && a.Board_Category == "N"
                                  select a).FirstAsync();
                }
                item.Top_YN = model.Top_YN ? "Y" : "N";
